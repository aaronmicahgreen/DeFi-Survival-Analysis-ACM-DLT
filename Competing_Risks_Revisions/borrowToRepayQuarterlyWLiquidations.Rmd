...
sampledTransactions <- sampledTransactions %>%
  mutate(reserve = case_when(type == "liquidation" ~ principalReserve,
                             TRUE ~ reserve)) %>%
  mutate(type = case_when(type == "liquidation" ~ "repay",
                          TRUE ~ type))
...
source("../First_Draft_Code/createSurvDataTable.R")
...
source("../First_Draft_Code/createSurvPlot.R")
...
image
ggsave(file="./Competing_Risks_Figures/3.4_borrowToRepayQuarterly.pdf", plot=image$plot, width=pubWidth, height=pubHeight)
