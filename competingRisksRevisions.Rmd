...
tidycmprsk::crr(Surv(time/86400, endpoint) ~ coinType, data = smallSet)

tidycmprsk::cuminc(Surv(time/86400, endpoint) ~ coinType, data = smallSet) %>% 
  ggcuminc() + 
  labs(x = "Days") + 
  add_confidence_interval() + 
  ggtitle(label = "How Does Coin Type Affect Repayment time? - Cumulative Incidence Function")



# group <- rep(1, nrow(smallSet))
# fit <- cuminc(smallSet$time, smallSet$cause, group, strata = smallSet$coinType, cencode = 1)
# plot(fit, main = 'cmprsk', xlab = 'Days')
# summary(fit)
# 
# 
...
#                   model="prop")
# summary(test)
# pout1 <- predict(test, X = 2)
# pout1 <- predict(test, X = 3)
# plot(pout1, xlim = c(0, 570), xlab = 'Days', main = 'timereg', uniform = 0, se = 0)
```
