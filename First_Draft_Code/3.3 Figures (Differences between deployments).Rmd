...
v3Transactions <- v3Transactions %>%
  select(-borrowRateMode)
mainnetAndV3Transactions <- bind_rows(mainnetTransactions, v3Transactions)
```

```{r}
indexEvents = c("deposit")
outcomeEvents = c("borrow")
joinFeatures = c("user")
obsPeriod = c(0, -1)
indexCovs = c("Deployment" = "deployment")
outcomeCovs = c()
...
kmXLab = "Time (Days)"
kmYLab = "Probability of No Borrows"
kmLegendTitle = "Aave Market"
kmLegendPosition = c(.8, .8)
kmXRange = c(0, 200)


survivalData[[strata]] <- as.factor(survivalData[[strata]]) %>%
  fct_relevel(c("Mainnet", "Arbitrum", "Avalanche", "Fantom", "Harmony", "Optimism", "Polygon"))
survDataTable <- createSurvDataTable(survivalData, strata, rmstMaxTime = 203)

kmLegendLabels = paste0(survDataTable[[strata]], " (", format(round(survDataTable$Percentage, 2)), "%)")

survPlot <- createSurvPlot(survivalData,
                           strata,
                           "How Quickly Do Users Borrow Funds After Opening An Account?",
                           kmXLab,
                           kmYLab,
                           kmLegendTitle,
                           kmLegendLabels,
...
image=ggpubr::ggpar(survPlot,
              ...
              palette = pubColorScheme)
ggsave(file="./Images/3.3_depositToBorrowMarket.pdf", plot=image$plot, width=pubWidth, height=pubHeight)
...
survPlot <- createSurvPlot(survivalData,
                           strata,
                           "How Quickly Do Users Redeem Funds After Repaying Loans?",
                           kmXLab,
                           kmYLab,
                           kmLegendTitle,
                           kmLegendLabels,
...
image=ggpubr::ggpar(survPlot,
              ...
              font.subtitle = list(size = pubFontSize),
              palette = pubColorScheme)
ggsave(file="./Images/3.3_repayToRedeemMarket.pdf", plot=image$plot, width=pubWidth, height=pubHeight)
